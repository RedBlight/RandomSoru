...
$_ders = isset($_GET['ders']) ? urlencode(htmlentities($_GET['ders'])) : 'nonvalid';
$_konu = isset($_GET['konu']) ? urlencode(htmlentities($_GET['konu'])) : 'nonvalid';

$html_soru = "";

if( $DBcon = mysql_pconnect("##DBserver##", "##DBuser##", "##DBpass##") ) {
  if( mysql_select_db("##DBname##", $DBcon) ) {
    if( $data = mysql_fetch_array(mysql_query("SELECT cookey, adminstate FROM userdata WHERE id = '$_uid'")) ) { $nomember = false; }
	else { $nomember = true; }
      if( $nomember || validate_password($_cookey, $data['cookey']) ) { setcookie("cookey", $cookey, 2147483000, '/');
		if( $nomember || mysql_query( "UPDATE userdata SET cookey = '".create_hash($cookey, 64)."', clickcount = clickcount+1 WHERE id = '$_uid'" ) ) {
		  if( $konuData = mysql_fetch_array(mysql_query("SELECT sorucount, sorulist FROM konudata WHERE ders = '$_ders' AND ind = '$_konu'")) ) {
		    if( $konuData['sorucount'] > 0 ) {
			  $soruList = explode(":", $konuData['sorulist']);
			  $pickId = $soruList[mt_rand(0, ($konuData['sorucount']-1))];
			  if( $soruCode = mysql_fetch_array(mysql_query("SELECT code FROM sorudata WHERE id = '$pickId'")) ) {
			   
// CODE ARRAY DEFINES
define( "HAVE_GRAPH",		0,	true);
define( "HAVE_TOPTEXT",		1,	true);
define( "HAVE_SOLUTION",	2,	true);
define( "CODE",				3,	true);
define( "GRAPH_X",			4,	true);
define( "GRAPH_Y",			5,	true);
define( "GRAPH_W",			6,	true);
define( "GRAPH_H",			7,	true);
define( "GRAPH",			8,	true);
define( "TOPTEXT_X",		9,	true);
define( "TOPTEXT_Y",		10,	true);
define( "TOPTEXT_W",		11,	true);
define( "TOPTEXT_H",		12,	true);
define( "TOPTEXT",			13,	true);
define( "TEXT",				14,	true);
define( "CHL",				15,	true);
define( "CHT",				16,	true);
define( "CHF1",				17,	true);
...
//MAKE CODE ARRAY
/*$removeList = array(	'@<script[^>]*?>.*?</script>@si',  // Strip out javascript 
						'@<style[^>]*?>.*?</style>@siU',   // Strip style tags properly 
						'@<![\s\S]*?--[ \t\n\r]*>@'        // Strip multi-line comments including CDATA 
); */
$soruval = explode(":", $soruCode['code']);
for($i=0; $i<22; $i++)
{
	//$soruval[$i] = preg_replace( $removeList, '', base64_decode($soruval[$i]) );
...
function ShowAnswer(opt) { if(!inAnswer)
{
	...
	foreach($Seval->var_token as $varname)
	{
		...
		$i++;
			  ...
			  } else { $html_soru = "<span class='spanred'>Hata! Soru bulunamadı."; }
		    } else { $html_soru = "<span>Bu konu için malesef daha soru eklenmemiş."; }
...
//echo trim( preg_replace('/\s+/', ' ', $html_soru) );
echo $html_soru;
